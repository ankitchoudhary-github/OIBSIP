...
import { ArrowLeft, Loader2, MapPin, ShieldCheck, ShoppingBag, ShoppingCart } from "lucide-react";
  ...
  <div>
    ...
    {props.type === "textarea" ? (
      <textarea
        id={id}
        name={id}
        className="w-full resize-none rounded-2xl border border-black/10 bg-black/2 px-4 py-3 text-sm outline-none transition focus:border-pizzaro-red focus:bg-white focus:ring-4 focus:ring-pizzaro-red/10"
        {...props}
      />
    ) : (
      <input
        id={id}
        name={id}
        className="w-full rounded-2xl border border-black/10 bg-black/2 px-4 py-3 text-sm outline-none transition focus:border-pizzaro-red focus:bg-white focus:ring-4 focus:ring-pizzaro-red/10"
        {...props}
      />
    )}
  </div>
...
const Checkout = () => {
    ...
    try {
      ...
      const paymentResponse = await fetch(
        `${import.meta.env.VITE_API_URL}/api/payments/create`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ orderId: mongoOrder._id }),
        }
      );
      ...
      const options = {
        ...
        name: "Pizzaro",
        description: "Pizzaro Pizza Order",
        order_id: paymentData.payment.orderId,
        prefill: {
          name: name.trim(),
          contact: phone.trim(),
        },
        ...
        handler: async function (response) {
          try {
            ...
            clearCart();
            // Transition directly to the Order Confirmation view
  ...
  if (cartItems.length === 0) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-pizzaro-cream px-6 py-32">
        <div className="text-center">
          <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-pizzaro-red/10 text-pizzaro-red">
          ...
          <button
            onClick={() => navigate("/")}
            className="mt-8 rounded-full bg-pizzaro-red px-8 py-4 font-semibold text-white transition hover:bg-red-700"
          >
            Browse Menu
          </button>
        </div>
    ...
    <main className="min-h-screen bg-pizzaro-cream px-6 pb-20 pt-32">
      ...
      >
        ...
        <button
          ...
          className="group mb-8 flex items-center gap-2 text-sm font-semibold text-pizzaro-dark transition hover:text-pizzaro-red"
        >
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <section className="rounded-4xl bg-white p-6 shadow-pizzaro sm:p-8">
            <div className="mb-8 flex items-center gap-3">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-pizzaro-red/10 text-pizzaro-red">
                <MapPin size={20} />
              </div>
              <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="pt-2">
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-full bg-pizzaro-dark px-6 py-4 text-sm font-semibold text-white transition hover:bg-pizzaro-red disabled:cursor-not-allowed disabled:opacity-80"
                >
                  {isProcessingPayment ? (
                    <>
                ...
                <div className="mt-4 flex items-center justify-center gap-2 text-xs text-pizzaro-muted">
          ...
          <section className="h-fit rounded-4xl bg-white p-6 shadow-pizzaro sm:p-8">
            ...
            <div className="space-y-4">
              {cartItems.map((item) => (
                <div
                  key={item.id}
                  className="rounded-2xl bg-pizzaro-cream p-4 transition-colors hover:bg-[#f5e8dc]"
                >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {item.type === "custom" && (
                        <div className="mt-2 text-xs leading-5 text-pizzaro-muted">
                          <p>
                            {item.base} &middot; {item.sauce} &middot;{" "}
                            {item.cheese}
                          </p>
                          {item.vegetables?.length > 0 && (
                            <p>{item.vegetables.join(" · ")}</p>
                          )}
                        </div>
                      )}
                    </div>
              ...
              <div className="border-t border-black/10 pt-5">
                ...
                <div className="flex items-end justify-between">
                  <span className="text-sm font-medium text-pizzaro-muted">
                    Total
                  </span>
